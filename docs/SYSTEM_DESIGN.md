...
| flux7-mesh | mem7 | Approval/rejection decisions | On approval resolve (opt-in) |
...
│   └── traces/ (JSONL)
├── mem7 (Go binary, MCP stdio via flux7-mesh)
...
### Phase 1: Decision write path (flux7-mesh → mem7) ✓
...
flux7-mesh queries mem7 before submitting to the approval queue. This is the built-in Level 1 supervisor — a pre-filter that handles routine patterns.
...
### Phase 3: flux7-console reads mem7 via SDK
...
- flux7-mesh handles the mem7 write (phase 1), flux7-console doesn't write to flux7-memory directly
